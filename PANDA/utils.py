...
import faiss
import ResNet
from prepare_data import XrayDataset, CSV_BASE_PATH, IMG_PATH
...
transform_color_pil = transforms.Compose([transforms.Resize(256),
                                          transforms.CenterCrop(224),
                                          transforms.ToTensor(),
                                          transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])])
...
def get_loaders(dataset, label_class, batch_size):
    if dataset in ['cifar10', 'fashion', 'custom']:
        if dataset == "cifar10":
            ...
            transform = transform_color_pil
            trainset = XrayDataset(csv_file=CSV_BASE_PATH + 'train_small.csv', root_dir=IMG_PATH, transform=transform)
            testset = XrayDataset(csv_file=CSV_BASE_PATH + 'test_small.csv', root_dir=IMG_PATH, transform=transform)

        idx = np.array(trainset.targets) == label_class
        testset.targets = [int(t != label_class) for t in testset.targets]
        trainset.data = np.array(trainset.data[idx])
